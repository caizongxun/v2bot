"""
Hugging Face 數據集上傳器 - 修正版

自動离断 huggingface-hub 版本並使用正確的參數

使用方式:
!pip install -q huggingface-hub
import requests
exec(requests.get('https://raw.githubusercontent.com/caizongxun/v2bot/main/data_fetcher/hf_uploader_fixed.py').text)
"""
...
def upload_to_huggingface(token, username, repo_name, data_dir):
    ...
    try:
        ...
        try:
            ...
            log("Multi-commit upload used", "INFO")
        except TypeError as e:
            if "multi_commit" in str(e):
                log("multi_commit not supported, using standard upload", "WARNING")
                # 使用標準上傳（子目錄正序上傳）
                api.upload_folder(
                    folder_path=data_dir,
                    repo_id=repo_id,
