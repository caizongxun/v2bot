...
subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", 
                       "numpy==2.1.3", "pandas==2.2.2", "scikit-learn==1.6.0"])
...
for col in formula_cols:
    ...
    iqr = q3 - q1
    outliers = ((merged_df[col] < q1 - 3*iqr) | (merged_df[col] > q3 + 3*iqr)).sum()
    if outliers > 0:
...
print(f"[Scaling] Mean: {X_scaled.mean(axis=0)[:5].round(4)}")
print(f"[Scaling] Std:  {X_scaled.std(axis=0)[:5].round(4)}")
...
train_info = {
    ...
    'feature_scaling': {
        ...
        'mean': X_scaled.mean(axis=0).tolist()[:10],  # 前10个
...
3. Data Cleaning:
   - Rows with missing values: {initial_rows - final_rows}
...
{chr(10).join(['  ' + col for col in feature_cols[:20]])}
  ... and {len(feature_cols) - 20} more columns
...
print("\n[STEP 13] Data statistics summary...")
...
print("\nFirst 15 features:")
print(feature_stats.head(15).round(4))
